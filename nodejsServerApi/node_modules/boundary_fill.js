...
class Player {
    ...
    async fillArea() {
        ...
        while (this.activTail.length != 0) {
            curr = this.activTail[this.activTail.length - 1];
            this.activTail.pop();
            if (this.isOutOfArenaGrid(curr)) continue;
            if (this.arena.owner[curr.x][curr.y] == this.id) {
                this.arena.setAsOwner(curr, this);
                if (this.arena.owner[curr.x][curr.y].isTail) {

                }
            }
        }
        console.log(this.activTail);
    ...
    async updatePosition(newLocation) {
        ...
        if (await this.isOutOfArenaGrid(gridCoords) === true) {
            ...
            if (this.arena.fields[gridCoords.x][gridCoords.y].owner == this.id) {
                ...
                if (this.arena.fields[gridCoords.x][gridCoords.y].isTail == false) {
                    console.log("joining tale?");
