...
class Player {
    ...
    async fillArea() {
        ...
        for (let i = 0; i < this.arena.gridSize.x; i++) {
            ...
            stack.push(new Point(i, this.arena.gridSize.y - 1));
        }
        for (let i = 0; i < this.arena.gridSize.y; i++) {
            stack.push(new Point(0, i));
            stack.push(new Point(this.arena.gridSize.x, i));
        }
        ...
        while (stack.length > 0) {
            ...
            stack.pop();
            if (await this.isOutOfArenaGrid(curr)) continue;
    ...
    async fillBetween(prevPos, currPos) {
        if (prevPos.x == currPos.x) {
            let min = Math.min(prevPos.y, currPos.y);
            let max = Math.max(prevPos.y, currPos.y);
            for (let i = min; i <= max; i++) {
                this.processPosition(prevPos.x, i);
            }
            return;
        }
        if (prevPos.y == currPos.y) {
            let min = Math.min(prevPos.x, currPos.x);
            let max = Math.max(prevPos.x, currPos.x);
            for (let i = min; i <= max; i++) {
                this.processPosition(i, prevPos.y);
            }
            return;
        }

        let steep = new Point((currPos.x + prevPos.x) / parseFloat(2), ((currPos.y + prevPos.y) / parseFloat(2)));
        if (steep.x == Math.floor(steep.x) && steep.y == Math.floor(steep.y)) {
            if (prevPos.x < currPos.x) {
                prevPos.x++;
            }
            else {
        ...
        steep.x = Math.floor(steep.x);
        steep.y = Math.floor(steep.y);

        let curr = new Point(prevPos.x, prevPos.y);
        this.processPosition(prevPos);
        console.log("steep ", steep);
        while (await prevPos.manhattanDistance(curr) <= await prevPos.manhattanDistance(currPos)) {
            console.log(prevPos, curr, currPos);
            console.log(await prevPos.manhattanDistance(curr), await prevPos.manhattanDistance(currPos));
            for (let i = 0; i < steep.y; i++) {
                curr.y++;
                if (prevPos.manhattanDistance(curr) > prevPos.manhattanDistance(currPos)) break;
                this.processPosition(curr);
            }
            for (let i = 0; i < steep.x; i++) {
                curr.x++;
                if (prevPos.manhattanDistance(curr) > prevPos.manhattanDistance(currPos)) break;
                this.processPosition(curr);
            }
        }
        this.processPosition(currPos);
    }

    async processPosition(gridCoords) {
        // is on own territory
        console.log("processing ", gridCoords);
