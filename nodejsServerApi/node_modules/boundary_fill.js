...
class Player {
    ...
    async fillBetween(prevPos, currPos) {
        let steep = new Point(currPos.x - prevPos.x, currPos.y - prevPos.y);
        if (steep.x % 2 == 1 || steep.y % 2 == 1) {
        ...
        if (steep.x == 0) {
            ...
            for (let i = min; i <= max; i++) {
                this.processPosition(new Point(prevPos.x, i));
            }
            return;
        }
        if (steep.y == 0) {
            ...
            for (let i = min; i <= max; i++) {
                this.processPosition(new Point(i, prevPos.y));
            }
            return;
        }
    ...
    async processPosition(gridCoords) {
        // is on own territory
        console.log("processing ", gridCoords.x, " ", gridCoords.y);
